...
class Nova_Addons_Portfolio {
	...
	public function __construct() {
		...
		// Add image size
		add_image_size( 'nova-portfolio', 451, 451, true );
		add_image_size( 'nova-portfolio-large', 912, 912, true );
		add_image_size( 'nova-portfolio-wide', 912, 451, true );
		add_image_size( 'nova-portfolio-masonry', 565, 10000, false );
		add_image_size( 'nova-portfolio-single', 1140, 740, true );
		...
		add_filter( 'nova_breadcrumbs_args', array( $this, 'breadcrumb' ) );
	...
	public function register_post_type() {
		...
		$portfolio_permalink = empty( $permalinks['portfolio_base'] ) ? _x( 'portfolio', 'slug', 'nova' ) : $permalinks['portfolio_base'];
		$portfolio_page_id   = get_option( $this->option . '_page_id' );
		$portfolio_type_base = empty( $permalinks['portfolio_type_base'] ) ? _x( 'portfolio-type', 'slug', 'nova' ) : $permalinks['portfolio_type_base'];

		register_post_type( $this->post_type, array(
			'description'         => esc_html__( 'Portfolio Items', 'nova' ),
			'labels'              => array(
				'name'                  => esc_html__( 'Portfolio', 'nova' ),
				'singular_name'         => esc_html__( 'Project', 'nova' ),
				'menu_name'             => esc_html__( 'Portfolio', 'nova' ),
				'all_items'             => esc_html__( 'All Projects', 'nova' ),
				'add_new'               => esc_html__( 'Add New', 'nova' ),
				'add_new_item'          => esc_html__( 'Add New Project', 'nova' ),
				'edit_item'             => esc_html__( 'Edit Project', 'nova' ),
				'new_item'              => esc_html__( 'New Project', 'nova' ),
				'view_item'             => esc_html__( 'View Project', 'nova' ),
				'search_items'          => esc_html__( 'Search Projects', 'nova' ),
				'not_found'             => esc_html__( 'No Projects found', 'nova' ),
				'not_found_in_trash'    => esc_html__( 'No Projects found in Trash', 'nova' ),
				'filter_items_list'     => esc_html__( 'Filter projects list', 'nova' ),
				'items_list_navigation' => esc_html__( 'Project list navigation', 'nova' ),
				'items_list'            => esc_html__( 'Projects list', 'nova' ),
			),
			'supports'            => array(
				'title',
				'editor',
		...
		register_taxonomy( $this->taxonomy_type, $this->post_type, array(
			'hierarchical'      => true,
			'labels'            => array(
				'name'                  => esc_html__( 'Project Types', 'nova' ),
				'singular_name'         => esc_html__( 'Project Type', 'nova' ),
				'menu_name'             => esc_html__( 'Project Types', 'nova' ),
				'all_items'             => esc_html__( 'All Project Types', 'nova' ),
				'edit_item'             => esc_html__( 'Edit Project Type', 'nova' ),
				'view_item'             => esc_html__( 'View Project Type', 'nova' ),
				'update_item'           => esc_html__( 'Update Project Type', 'nova' ),
				'add_new_item'          => esc_html__( 'Add New Project Type', 'nova' ),
				'new_item_name'         => esc_html__( 'New Project Type Name', 'nova' ),
				'parent_item'           => esc_html__( 'Parent Project Type', 'nova' ),
				'parent_item_colon'     => esc_html__( 'Parent Project Type:', 'nova' ),
				'search_items'          => esc_html__( 'Search Project Types', 'nova' ),
				'items_list_navigation' => esc_html__( 'Project type list navigation', 'nova' ),
				'items_list'            => esc_html__( 'Project type list', 'nova' ),
			),
			'public'            => true,
			'show_ui'           => true,
			'show_in_nav_menus' => true,
	...
	public function edit_admin_columns( $columns ) {
		...
		$columns['title'] = esc_html__( 'Project', 'nova' );
	...
	public function settings_api_init() {
		add_settings_section(
			'nova_portfolio_section',
			'<span id="portfolio-options">' . esc_html__( 'Portfolio', 'nova' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="portfolio-options">' . esc_html__( 'Portfolio Projects', 'nova' ) . '</span>',
		...
		if ( get_option( $this->option ) ) {
			...
			add_settings_field(
				...
				'<span class="portfolio-options">' . esc_html__( 'Portfolio page', 'nova' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_items_per_page">' . esc_html__( 'Portfolio items show at most', 'nova' ) . '</label>',
			...
			add_settings_section(
				'nova_portfolio_section',
				'<span id="portfolio-options">' . esc_html__( 'Portfolio Item Permalink', 'nova' ) . '</span>',
			...
			add_settings_field(
				'portfolio_type_slug',
				'<label for="portfolio_type_slug">' . esc_html__( 'Portfolio type base', 'nova' ) . '</label>',
	...
	public function writing_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to display custom types of content on your site', 'nova' ); ?>
		</p>
		<?php
	}
	...
	public function reading_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control custom post type content', 'nova' ); ?>
		</p>
		<?php
	}
	...
	public function permalink_section_html() {
		...
		$portfolio_page_id = get_option( $this->option . '_page_id' );
		$base_slug         = urldecode( ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'nova' ) );
		$portfolio_base    = _x( 'portfolio', 'Default slug', 'nova' );
		...
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control the permalink used specifically for portfolio.', 'nova' ); ?>
		</p>

		<table class="form-table nova-portfolio-permalink-structure">
			<tbody>
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="nova-portfolio-base" /> <?php esc_html_e( 'Default', 'nova' ); ?>
					</label>
				</th>
				<td>
				...
				<tr>
					<th>
						<label><input name="portfolio_permalink" type="radio"
									  ...
									  class="nova-portfolio-base" /> <?php esc_html_e( 'Portfolio base', 'nova' ); ?>
						</label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="nova-portfolio-base" /> <?php esc_html_e( 'Portfolio base with type', 'nova' ); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" id="nova_portfolio_custom_selection" type="radio"
								  value="custom" <?php checked( in_array( $portfolio_permalink, $structures ), false ); ?> /> <?php esc_html_e( 'Custom Base', 'nova' ); ?>
					</label>
				</th>
				<td>
					<code><?php echo esc_html( home_url() ); ?></code>
					<input name="portfolio_permalink_structure" id="nova_portfolio_permalink_structure" type="text"
		...
		<script type="text/javascript">
			jQuery( function () {
				jQuery( 'input.nova-portfolio-base' ).change( function () {
					jQuery( '#nova_portfolio_permalink_structure' ).val( jQuery( this ).val() );
				} );
				jQuery( '.permalink-structure input' ).change( function () {
					jQuery( '.nova-portfolio-permalink-structure' ).find( 'code.non-default-example, code.default-example' ).hide();
					if ( jQuery( this ).val() ) {
						jQuery( '.nova-portfolio-permalink-structure code.non-default-example' ).show();
						jQuery( '.nova-portfolio-permalink-structure input' ).removeAttr( 'disabled' );
					} else {
						jQuery( '.nova-portfolio-permalink-structure code.default-example' ).show();
						jQuery( '.nova-portfolio-permalink-structure input:eq(0)' ).click();
						jQuery( '.nova-portfolio-permalink-structure input' ).attr( 'disabled', 'disabled' );
					}
				} );
				jQuery( '.permalink-structure input:checked' ).change();
				jQuery( '#nova_portfolio_permalink_structure' ).focus( function () {
					jQuery( '#nova_portfolio_custom_selection' ).click();
				} );
			} );
		</script>
		<?php
	...
	public function enable_field_html() {
		...
		<label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Enable Portfolio Projects for this site.', 'nova' ); ?>
		</label>
	...
	public function per_page_field_html() {
		...
		<label for="portfolio_posts_per_page">
			...
			<?php _ex( 'items', 'Portfolio items per page', 'nova' ) ?>
		</label>
	...
	public function portfolio_type_slug_field_html() {
		...
		<input name="portfolio_type_slug" id="portfolio_type_slug" type="text"
			   ...
			   placeholder="<?php echo esc_attr( _x( 'portfolio-type', 'Portfolio type base', 'nova' ) ) ?>"
		...
		if ( isset( $_POST['portfolio_permalink'] ) ) {
			...
			if ( 'custom' === $portfolio_permalink ) {
				...
				if ( '%portfolio_type%' == $portfolio_permalink ) {
					$portfolio_permalink = '/' . _x( 'portfolio', 'slug', 'nova' ) . '/' . $portfolio_permalink;
				}
			...
			$portfolio_page_id   = get_option( $this->option . '_page_id' );
			$portfolio_permalink = ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'nova' );
	...
	public function portfolio_post_type_link( $permalink, $post ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$portfolio_type = _x( 'uncategorized', 'slug', 'nova' );
		}
	...
	public function breadcrumb( $args ) {
		...
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$args['labels']['archive'] = _x( 'Portfolio', 'Portfolio post type breadcrumb', 'nova' );
			}
		}
	...
	public function archive_title( $title ) {
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$title = _x( 'Portfolio', 'Portfolio post type breadcrumb', 'nova' );
			}
		}
