...
if(!empty($title)){
    ...
    if( $read_more == 'title' && $a_link_open != '') {
        $box_heading_html = '<div class="box-heading"><h5 class="'.esc_attr($title_class).'" style="'. esc_attr( implode(';', $titleCssInline)).'" '.$titleHtmlAtts.'>' . $a_link_open . esc_html($title) . $a_link_close . '</h5></div>';
    }else{
...
<div id="<?php echo esc_attr($unique_id)?>" class="<?php echo esc_attr($css_class)?>">
    <div class="icon-boxes-inner"><?php echo $inner_html;?><?php
        if( $read_more == 'box' && $a_link_open != '') {
            echo $a_link_open . $a_link_close;
        }
        ?></div>
</div>
