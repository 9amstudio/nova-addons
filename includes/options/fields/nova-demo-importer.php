 ...
 *
 */
class Novaworks_Framework_Field_Nova_Demo_Importer extends Novaworks_Framework_Field_Base {
		...
		if ( isset( $this->field['demo'] ) && is_array($this->field['demo'] ) ) {
			...
			foreach( $this->field['demo'] as $key => $demo ) {
				...
				if ( isset( $demo_installed[$key] ) ) {
					$class .= ' imported';
					$more_detail = esc_html__( 'Demo Imported', 'nova' );
					$btn_import = esc_html__( 'Imported', 'nova' );
					...
					$class .= ' not-imported';
					$more_detail = esc_html__( 'Import Demo', 'nova' );
					$btn_import = esc_html__( 'Import Demo', 'nova' );
				...
				<div class="<?php echo esc_attr( $class ); ?>" data-demo-id="<?php echo esc_attr( $key ); ?>">
					...
					<div class="theme-id-container">
						<h2 class="theme-name"><span><?php esc_html_e( 'Active:', 'nova' );?></span> <?php echo esc_html( $demo['title'] )?></h2>
						<div class="theme-actions">
							<div class="importer-buttons">
								<span class="spinner"><?php esc_html_e( 'Please Wait...', 'nova' ); ?></span>
								<span data-title="<?php echo esc_attr( 'Import Demo Content?', 'nova' ); ?>" class="<?php echo esc_attr( $btn_import_class ); ?>"><?php echo esc_html( $btn_import ); ?></span>
								<span data-title="<?php echo esc_attr( 'Re-Import Content?', 'nova' ); ?>" class="<?php echo esc_attr( $btn_reimport_class ); ?>"><?php esc_html_e( 'Re-Import', 'nova' ); ?></span>
							</div>
						</div>
						<div class="theme-extra-options hidden">
							<label>
								<input class="checkbox extra-import-options" type="checkbox" /><?php esc_html_e( 'Without Content', 'nova' ); ?>
							</label>
						</div>
					</div>
				</div>
