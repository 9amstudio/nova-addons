...
class Novaworks_Framework_Field_Base {
    ...
    public function element_after() {
        ...
        $out  = ( isset( $this->field['info'] ) ) ? '<p class="nova-text-desc">'. $this->field['info'] .'</p>' : '';
    ...
    public function element_debug() {
        ...
        if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'NOVA_OPTIONS_DEBUG' ) && NOVA_OPTIONS_DEBUG ) ) {

            $value = $this->element_value();

            $out .= "<pre>";
            $out .= "<strong>". esc_html__( 'CONFIG', 'nova' ) .":</strong>";
            $out .= "\n";
            ob_start();
            var_export( $this->field );
            $out .= htmlspecialchars( ob_get_clean() );
            $out .= "\n\n";
            $out .= "<strong>". esc_html__( 'USAGE', 'nova' ) .":</strong>";
            $out .= "\n";
            $out .= ( isset( $this->field['id'] ) ) ? "nova_fw_get_option( '". $this->field['id'] ."' );" : '';

            if( ! empty( $value ) ) {
                $out .= "\n\n";
                $out .= "<strong>". esc_html__( 'VALUE', 'nova' ) .":</strong>";
                $out .= "\n";
                ob_start();
                var_export( $value );
                $out .= htmlspecialchars( ob_get_clean() );
        ...
        if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'NOVA_OPTIONS_DEBUG_LIGHT' ) && NOVA_OPTIONS_DEBUG_LIGHT ) ) {
            ...
            $out .= ( isset( $this->field['id'] ) ) ? "nova_fw_get_option( '". $this->field['id'] ."' );" : '';
            $out .= "\n";
            $out .= "<strong>". esc_html__( 'ID', 'nova' ) .":</strong>";
            $out .= "\n";
    ...
    public function element_get_error() {
        ...
        if( ! empty( $nova_errors ) ) {
            foreach ( $nova_errors as $key => $value ) {
                if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
                    $out .= '<p class="nova-text-warning">'. $value['message'] .'</p>';
                }
            }
        }
    ...
    public function element_help() {
        return ( isset( $this->field['help'] ) ) ? '<span class="nova-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
    }
