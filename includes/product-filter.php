...
add_action(
	...
	register_widget( 'Nova_Product_Filter_Widget' );
} );

/**
 * Class product filter widget
 */
class Nova_Product_Filter_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		...
		parent::__construct(
			'nova-product-filter',
			esc_html__( 'Nova Product Filter', 'nova' ),
			array(
				'classname'   => 'woocommerce nova-product-filter-widget',
				'description' => esc_html__( 'Display product filters', 'nova' ),
			),
			array( 'width' => 780 )
		);
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'nova-product-filter', NOVA_ADDONS_URL . 'assets/js/product-filter.js', array(
			'jquery-ui-slider',
			'wp-util',
			'jquery-serialize-object',
		), '20160623', true );

		wp_localize_script( 'nova-product-filter', 'novaFilter', array(
			'currency' => array(
				'position' => get_option( 'woocommerce_currency_pos' ),
				'symbol'   => get_woocommerce_currency_symbol(),
			),
			'selector' => apply_filters( 'nova_product_filter_ajax_selector', array(
	...
	public function widget( $args, $instance ) {
		...
		echo '<input type="submit" value="' . esc_attr( apply_filters( 'nova_product_filter_button_text', __( 'Filter', 'nova' ) ) ) . '" class="btn button filter-button">';
	...
	public function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'ajax' ) ); ?>"><?php esc_html_e( 'Use ajax for filtering', 'nova' ); ?></label>
		</p>

		<hr>
		<p></p>

		<div class="nova-product-filters">
			<p class="no-filter <?php echo empty( $instance['filter'] ) ? '' : 'hidden' ?>"><?php esc_html_e( 'There is no filter yet.', 'nova' ) ?></p>

			<?php foreach ( (array) $instance['filter'] as $index => $filter ) : ?>

				<div class="nova-product-filter-fields">
					<div class="name">
						<label>
							<?php esc_html_e( 'Filter Name', 'nova' ) ?>
					...
					<div class="source">
						<label>
							...
							<select name="<?php echo esc_attr( $this->get_field_name( "filter[$index]" ) ) ?>[attribute]" class="widefat <?php echo 'attribute' == $filter['source'] ? '' : 'hidden' ?>">
								<?php
								foreach ( Nova_Product_Filter()->attributes as $attribute => $name ) {
					...
					<div class="display">
						<label>
							<?php esc_html_e( 'Display Type', 'nova' ) ?>
							<select name="<?php echo esc_attr( $this->get_field_name( "filter[$index]" ) ) ?>[display]" class="widefat display-type">
								<?php
								foreach ( Nova_Product_Filter()->display[ $filter['source'] ] as $display => $name ) {
							...
							<select name="<?php echo esc_attr( $this->get_field_name( "filter[$index]" ) ) ?>[multiple]" class="widefat <?php echo 'attribute' != $filter['source'] || 'dropdown' == $filter['display'] ? 'hidden' : '' ?>">
								<option value="1" <?php selected( 1, $filter['multiple'] ) ?>><?php esc_html_e( 'Multiple select', 'nova' ) ?></option>
								<option value="0" <?php selected( 0, $filter['multiple'] ) ?>><?php esc_html_e( 'Single select', 'nova' ) ?></option>
							</select>
						</label>
					</div>
					<div class="actions">
						<a href="#" class="remove-filter dashicons dashicons-no-alt"><span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'nova' ) ?></span></a>
					</div>
				</div>
		...
		<p style="text-align: center">
			<a href="#" class="nova-product-filter-add-new" data-number="<?php echo esc_attr( $this->number ) ?>" data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ) ?>" data-count="<?php echo count( $instance['filter'] ) ?>">+ <?php esc_html_e( 'Add new filter', 'nova' ) ?></a>
		</p>
	...
	protected function display_filter( $filter = array() ) {
		...
		if ( 'attribute' == $filter['source'] ) {
			...
			$args['multiple'] = true;
			$args['all']      = sprintf( esc_html__( 'Any %s', 'nova' ), wc_attribute_label( $attr->attribute_label ) );
			...
			$args['all'] = sprintf( esc_html__( 'Select a %s', 'nova' ), $taxonomy->labels->singular_name );
		}
		...
		$args = apply_filters( 'nova_product_filter_args', $args, $filter );
		...
		$classes   = apply_filters( 'nova_product_single_filter_class', array_filter( $classes ), $filter );
		?>
	...
	protected function get_filter_options( $filter ) {
		...
		return apply_filters( 'nova_product_filter_options', $options, $filter );
	}
	...
	protected function display_slider( $args ) {
		...
		<div class="slider-amount">
			...
			<div class="slider-label hidden">
				<span class="range"><?php esc_html_e( 'Range:', 'nova' ) ?></span><span class="from"></span> &mdash;
				<span class="to"></span>
			</div>
			<div class="clear"></div>
		</div>
	...
	protected function display_auto( $args ) {
		...
		// Use select by default if plugin Nova Product Attribute Swatches is not installed
		if ( ! function_exists( 'is_plugin_active' ) ) {
			include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		}

		if ( is_plugin_active( 'variation-swatches-for-woocommerce' ) ) {
			$args['type'] = 'select';
		}
 ...
 * Plugin main class
 */
class Nova_Product_Filter {
	/**
	 * The single instance of the class
	 *
	 * @var Nova_Product_Filter
	 */
	...
	public function __construct() {
		$this->sources = apply_filters(
			'nova_product_filter_sources', array(
				'price'     => esc_html__( 'Price', 'nova' ),
				'category'  => esc_html__( 'Category', 'nova' ),
				'tag'       => esc_html__( 'Tag', 'nova' ),
				'attribute' => esc_html__( 'Attributes', 'nova' ),
			)
		);

		$this->display = apply_filters(
			'nova_product_filter_display', array(
				'price'     => array(
					'slider' => esc_html__( 'Slider', 'nova' ),
				),
				'category'  => array(
					'dropdown' => esc_html__( 'Dropdown', 'nova' ),
					'list'     => esc_html__( 'Vertical List', 'nova' ),
					'h-list'   => esc_html__( 'Horizontal List', 'nova' ),
				),
				'tag'       => array(
					'dropdown' => esc_html__( 'Dropdown', 'nova' ),
					'list'     => esc_html__( 'Vertical List', 'nova' ),
		...
		$this->attributes = apply_filters( 'nova_product_filter_attributes', $this->attributes );
	...
	public function enqueue_admin_scripts() {
		...
		wp_enqueue_script( 'nova-product-filter-admin', NOVA_ADDONS_URL . 'admin/js/nova-admin-product-filter.js', array( 'wp-util' ), '20160617', true );

		wp_localize_script(
			'nova-product-filter-admin', 'novaFilter', array(
	...
	public function add_form_templates() {
		...
		<script type="text/template" id="tmpl-nova-product-filter">
			<div class="nova-product-filter-fields">
				<div class="name">
					<label>
						<?php esc_html_e( 'Filter Name', 'nova' ) ?>
				...
				<div class="display">
					<label>
						...
						<select name="{{data.name}}[{{data.count}}][multiple]" class="widefat hidden">
							<option value="1"><?php esc_html_e( 'Multiple select', 'nova' ) ?></option>
							<option value="0"><?php esc_html_e( 'Single select', 'nova' ) ?></option>
						</select>
					</label>
				</div>
				<div class="actions">
		...
		<script type="text/template" id="tmpl-nova-product-filter-options">
 ...
 * @return Nova_Product_Filter
 */
function Nova_Product_Filter() {
	return Nova_Product_Filter::instance();
}
...
add_action( 'plugins_loaded', function () {
	if ( ! function_exists( 'WC' ) ) {
		add_action(
			...
			<div class="error">
				<p><?php esc_html_e( 'Nova Product Filter is enabled but not effective. It requires WooCommerce in order to work.', 'nova' ); ?></p>
			</div>
