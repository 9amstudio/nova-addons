...
/**
 * The Autoloader class for Nova Addons.
 */
class Novaworks_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'nova_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 'Novaworks_Framework_Field_Base' === $class_name ) {
            $path = wp_normalize_path( NOVA_ADDONS_DIR . 'includes/options/base-field.php' );
            return $path;
        }
        elseif ( 0 === stripos( $class_name, 'Novaworks_Framework_Field' ) ) {

            $path     = wp_normalize_path( NOVA_ADDONS_DIR . 'includes/options/fields/' );
            $filename = str_replace( 'Novaworks_Framework_Field_', '', $class_name );
        ...
        elseif ( 0 === stripos( $class_name, 'Novaworks' ) ) {
            $path     = wp_normalize_path( NOVA_ADDONS_DIR . 'includes/' );
            $filename = 'class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';

            $paths[] = $path . $filename;

            $substr   = str_replace( array( 'Novaworks_' ), '', $class_name );
