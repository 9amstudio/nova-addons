...
( function( $ ) {
	...
	$( function() {
		...
		function nova_render_hotspot_data_from_editor(){
			if( $( '[data-vc-shortcode-param-name="content"] .vc_textarea_html_content' ).val().length > 0 ) {
				var hotspotRegex = wp.shortcode.regexp( 'nova_hotspot' ),
					matches;
			...
			if( window.$hotSpotData.length > 0 ) {
				$.each( window.$hotSpotData, function( k, v ) {
					//hotspot
					$( '#nova_image_with_hotspots_preview' ).append( '<div class="hotspot" data-rel="' + ( k + 1 ) + '" style="top: ' + v.top + '; left: ' + v.left + ';">' + ( k + 1 ) + '</div>' );
					var $positionOptions = '';
		...
		function nova_update_hotspot_data( rel, x, y ) {
			...
			if( $( '[data-vc-shortcode="nova_image_with_hotspots"]' ).length > 0 && _content.length > 0 ) {
		...
		function nova_hotspot_make_draggable() {
			$( '#nova_image_with_hotspots_preview .hotspot:not(.ui-draggable)' ).draggable( {
				containment: 'parent',
				stop: function( event, ui ) {
					var $elm = $( this );
					...
					nova_update_hotspot_data( $elm.attr( 'data-rel' ), ( ( pos.left/parentSizes.width ) * 100) + '%', ( ( pos.top/parentSizes.height ) * 100 ) + '%' );
				}
			} );
		}

		////update numbers
		function nova_hotspot_update_numbers() {
			$( '#nova_image_with_hotspots_preview .hotspot' ).each( function( i ) {
				$( this ).html( i + 1 );
		...
		$( document )
			.on( 'click', '[data-vc-shortcode="nova_image_with_hotspots"] .vc_icon-remove', function( e ) {
				$( '#nova_image_with_hotspots_preview' ).addClass( 'no-img' );
			})
			.on( 'change', '[data-vc-shortcode="nova_image_with_hotspots"] .vc_shortcode-param[data-vc-shortcode-param-name="image"] .image', function( e ) {
				...
				if( image_id != '' ) {
					...
					$( '<img src="' + image_object.attributes.url + '" alt="preview"/>' ).appendTo( $( '#nova_image_with_hotspots_preview' ) );
					$( '#nova_image_with_hotspots_preview' ).removeClass( 'no-img' );
				}
			} )
			.on( 'click', '#nova_image_with_hotspots_preview:not(#nova_image_with_hotspots_preview .hotspot)', function( e ) {
				...
				var posX = $( this ).offset().left,
					...
					hotSpots = $( '#nova_image_with_hotspots_preview .hotspot' ).length;
				hotSpots++;
			...
			.on( 'click', '.hotspot-content a.delete', function( e ) {
				...
				nova_hotspot_update_numbers();
				//empty hotspotdata so list can be rebuilt inside next function
				window.$hotSpotData = [];
				$( '#nova_image_with_hotspots_preview .hotspot' ).each( function( i ) {
					nova_update_hotspot_data( i + 1, $( this )[0].style.left , $( this )[0].style.top );
				} );
				return false;
			} )
		...
		window.NovaVCAdminEditForm = function( tag ) {
			if( tag == 'vc_section' || tag == 'vc_row' ) {
				if( $( '.nova_enable_fp_fields input[type="radio"]:checked' ).val() == 'yes' ) {
					if( tag == 'vc_section' ){
